package io.github.treesitter.jtreesitter.zenc;

import java.lang.foreign.*;

public final class TreeSitterZenc {
    ...
    private static final TreeSitterZenc INSTANCE = new TreeSitterZenc();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterZenc() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_zenc");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-zenc");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-zenc");
